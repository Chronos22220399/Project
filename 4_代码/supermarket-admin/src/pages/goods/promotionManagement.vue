<template>
    <el-card>
        ...
        <div class="promotion-management">
            ...
            <el-dialog :title="isEdit ? '编辑促销活动' : '添加促销活动'" v-model="dialogVisible" width="600px">
...
const handleDialogConfirm = () => {
    isEdit.value ? updatePromotion() : createPromotion()
}
...
const updatePromotion = async () => {
    if (useMock.value) {
        const index = promotionList.value.findIndex(p => p.promotion_id === form.value.promotion_id)
        if (index !== -1) {
            promotionList.value[index] = { ...form.value }
        }
        dialogVisible.value = false
        resetForm()
    } else {
        try {
            const response = await axios.post('http://localhost:8080/api/promotion/update', form.value)
            if (response.data.code === 200) {
                dialogVisible.value = false
                resetForm()
                fetchPromotions()
