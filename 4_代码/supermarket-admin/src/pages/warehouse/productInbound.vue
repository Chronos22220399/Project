<!-- src/views/warehouse/Inbound.vue -->
<template>
    <el-card>
        <div class="toolbar">
            <el-button type="primary" @click="openDialog()">添加入库记录</el-button>
        </div>
...
const generateFakeData = () => {
    return Array.from({ length: 50 }, (_, index) => ({
        ...
        inboundTime: `2023-04-${String(index + 1).padStart(2, '0')} 12:00:00`,
        operator: `Operator ${index + 1}`
    }))
}

const tableData = ref<Inbound[]>(generateFakeData())  // 假数据
const dialogVisible = ref(false)
const dialogTitle = ref('添加入库记录')
const form = ref<Partial<Inbound>>({})
...
// 打开对话框
const openDialog = (row?: Inbound) => {
    dialogTitle.value = row ? '编辑入库记录' : '添加入库记录'
    form.value = row ? { ...row } : {}
...
const handleSave = () => {
    if (form.value.id) {
        ...
        if (index !== -1) tableData.value[index] = form.value as Inbound
    } else {
        form.value.id = Date.now()
        tableData.value.push(form.value as Inbound)
    }
