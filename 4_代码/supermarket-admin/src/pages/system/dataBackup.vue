<template>
    <el-card>
        ...
        <el-dialog :title="dialogTitle" v-model="dialogVisible">
            <el-form :model="form">
                <el-form-item label="备份时间"><el-date-picker v-model="form.backupTime" type="datetime" /></el-form-item>
                <el-form-item label="操作人"><el-input v-model="form.operator" /></el-form-item>
                <el-form-item label="文件位置"><el-input v-model="form.filePath" /></el-form-item>
            </el-form>
            <template #footer>
...
const generateFakeRecords = (): BackupRecord[] => {
    return Array.from({ length: 20 }, (_, i) => ({
        id: i + 1,
        backupTime: '2025-05-01 12:00:00',
        operator: `用户${i + 1}`,
        filePath: `/backup/data${i + 1}.zip`
    }))
}
...
const openDialog = (row?: BackupRecord) => {
...
const handleSave = () => {
    if (form.value.id) {
        ...
        if (index !== -1) {
            tableData.value[index] = { ...(form.value as BackupRecord) }
        }
    } else {
        const newId = Date.now()
        tableData.value.push({ ...(form.value as BackupRecord), id: newId })
    }
