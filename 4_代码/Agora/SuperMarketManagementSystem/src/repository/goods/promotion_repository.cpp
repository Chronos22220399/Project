...
select_ret_type<PromotionDTO> PromotionRepository::getById(in_id_type id) {
  return _select(db::promotion{}.id == id);
}

update_ret_type
PromotionRepository::updateById(in_id_type id,
                                const PromotionDTO &promotion_dto) {
  return _update(promotion_dto, db::promotion{}.id == id);
}

delete_ret_type PromotionRepository::removeById(in_id_type id) {
  return _remove(db::promotion{}.id == id);
}

bool PromotionRepository::existsById(in_id_type id) {
