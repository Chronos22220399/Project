...
inline void to_json(nlohmann::json &j, const GoodsPromotionDTO& goods_promotion_dto) {
    j = nlohmann::json{
        {"id", goods_promotion_dto.id},
{"goods_id", goods_promotion_dto.goods_id},
...
namespace model {
template <>
struct ReflectTable<GoodsPromotionDTO, db::goods_promotion> {
    static constexpr auto map_members = std::make_tuple(
        std::make_pair(&GoodsPromotionDTO::id, &db::goods_promotion::id),
std::make_pair(&GoodsPromotionDTO::goods_id, &db::goods_promotion::goods_id),
std::make_pair(&GoodsPromotionDTO::promotion_id, &db::goods_promotion::promotion_id),
std::make_pair(&GoodsPromotionDTO::discount_rate, &db::goods_promotion::discount_rate),
std::make_pair(&GoodsPromotionDTO::full_threshold, &db::goods_promotion::full_threshold),
std::make_pair(&GoodsPromotionDTO::reduce_amount, &db::goods_promotion::reduce_amount),
std::make_pair(&GoodsPromotionDTO::buy_quantity, &db::goods_promotion::buy_quantity),
std::make_pair(&GoodsPromotionDTO::gift_product, &db::goods_promotion::gift_product)
    );
};

// mapping
template <typename Goods_promotionRow> struct ReflectTableRow<GoodsPromotionDTO, Goods_promotionRow> {
