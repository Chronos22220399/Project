 ...
 * @author Ess
 */
[[nodiscard]] inline auto get_conn_pool_ptr(bool is_debug,
                                     const std::string &path_to_database) {
  sqlpp::sqlite3::connection_config config{};
  config.debug = is_debug;
  config.flags = SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE;
  config.path_to_database = path_to_database;
  auto config_ptr = std::make_shared<sqlpp::sqlite3::connection_config>(config);
 ...
 * @brief Offer user a convenient way to execute his/her database operation to prevents writing duplicate codes
 *
 * execute accept a connection_pool pointer wrapped by shared_ptr, an Operation and it's arguments
 *
 * @author Ess
 */
 ...
 * @brief Used to get the database data, the usage is specifing the DataType in template,
 * @brief then pass a connection_pool pointer, a table variable and a condition into method get_data
...
template <typename DataType> struct GenericDataGetter : utils::DataBaseHelper {
  ...
  [[nodiscard]] ReturnType get_data(const conn_pool_ptr_type &conn_pool_ptr,
    ...
    auto query = [](const pooled_conn_ptr_type &conn_, Table &&table_,
      ...
      ReturnType ret{};
      for (const auto &row : (*conn_)(select(all_of(table_))
                       .from(table_)
                       .where(std::forward<Condition>(condition_)))) {
        ret.emplace_back(DataTypeTraits<decltype(row), DataType>::get_args(row));
      }
      return ret;
    };
